...
def make_timeout_str(timeout_min: float):
    return f"{timeout_min}m"
...
def make_outpath(
    ...
    time_str = make_timeout_str(timeout_min)
                    ...
                    for _old_graph, old_graph_path in find_old_graph(graph_name):
                        ...
                        def doit(cache_graph_name, id, graph_path):
                            ...
                            out_path = make_outpath(
                                graph_name, id, dat_name, timeout_min
                            )
                            if os.path.isfile(out_path):
                                return

                            if (
                                call_shell(out_path, graph_path, dat_path, timeout_min)
                                == 0
                            ):
